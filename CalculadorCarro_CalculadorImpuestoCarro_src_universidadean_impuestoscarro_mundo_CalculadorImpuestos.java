...
public class CalculadorImpuestos {
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        for (int i = 0; i < vehiculos.length; i++) {
            if(maxCaro < vehiculos[i].darPrecio()){
                maxCaro = vehiculos[i].darPrecio();
                indice = i;
            }
            masCaro = vehiculos[indice];
        }
        return masCaro;
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for (int i = 0; i < vehiculos.length; i++) {
            if(vehiculos[i].darAnio()== buscado.darAnio()){
                buscado=vehiculos[i];
            }
    ...
    public double promedioPreciosVehiculos() {
        // TODO: el promedio del precio de todos los vehiculos
        double promedio = 0.0;
        for (int i = 0; i < vehiculos.length; i++) {
                promedio =promedio + vehiculos[i].darPrecio();
        }
        promedio =promedio / (double)vehiculos.length;
        return promedio;
    }
